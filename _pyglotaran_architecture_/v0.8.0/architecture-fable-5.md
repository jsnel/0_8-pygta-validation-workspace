...
This document describes the architecture of pyglotaran as it exists in this repository today
(branch `staging`, version `0.8.0.dev0`, after the internal rewrite merged in
...
All claims are grounded in the code. File paths are repository-relative. Statements that are
...
- `optimization/objective.py` already imports IO (`load_dataset`/`save_dataset`) because
  `OptimizationResult` lives there — accepted today, but do not add further IO into the
